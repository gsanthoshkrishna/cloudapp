...
def index():
    ...
    try:
        ...
        return render_template('appg.html', resources=resources, load_images=load_images)
...
def get_properties():
    ...
    try:
        ...
        if div_id == "load-container":
            ...
            JOIN resourse_prop rp ON tpl.prop_id = rp.res_prop_id
...
def get_resource_cost():
    ...
    try:
        cnx = cnxpool.get_connection()
        cursor = cnx.cursor(dictionary=True)
        print('--------in get resource cost------')
        ruid = request.args.get('resource_id')
        print(ruid)
        cnx = cnxpool.get_connection()
        cursor = cnx.cursor(dictionary=True)
       
        qry = "select cost from resource_cost where res_unique_id = '" + str(ruid) + "';"
        print("--------test-----------")
        print(qry)
        cursor.execute(qry)
        ...
        print("----------Exception in get resource cost-----------")
        print(e)
    finally:
        if cursor:
            cursor.close()
